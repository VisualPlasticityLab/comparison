...
normalised_pre = zscore(pre.F(:, section), 0, 2);
normalised_post = zscore(post.F(:, section), 0, 2);
...
cov_matrix_pre = cov(normalised_pre0');
...
cov_test_score_pre = normalised_pre0' * V_sorted;
cov_test_score_post = normalised_post0' * V_sorted;

%FOR BOTH SESSIONS TOGETHER
%covariance matrix, eigenvectors, eigenvalues
cov_matrix = cov(normalised0');
[Vec, Eig] = eig(cov_matrix);
%gives eigenvectors as columns in V and diagonal matrix E of eigen values 
[eigen_values_0, columns_0] = sort(diag(Eig),'descend');
%sorts eigen values in descending and gives columns indices
Eig_sorted = Eig(columns_0, columns_0);
%sorts E in descending order (PC1 to PC85)
Vec_sorted = Vec(:, columns_0);
%sorts E in descending order of columns (PC1 to PC85)
cov_test_score = normalised0' * Vec_sorted;


%section_length = size(normalised_pre, 2)
section_length = sum(section);
%section_length = nframe

c = turbo(nframe);
timepoint_indices = find(section == 1);
%BOTH SESSIONS
figure;hold on;
title('PCA scores');
%scores
subplot(1,2,1); hold on;
for i = 1:section_length;
    plot3(cov_test_score(i,1), cov_test_score(i,2), cov_test_score(i,3), '.', 'color', c(timepoint_indices(i),:))
%    stem3(cov_test_score(i,1), cov_test_score(i,2), cov_test_score(i,3), '.', 'color', c(timepoint_indices(i),:))
...
for i = (section_length+1):(section_length*2)
    plot3(cov_test_score(i,1), cov_test_score(i,2),cov_test_score(i,3), '.', 'color', c(timepoint_indices(i - section_length),:))
%    stem3(cov_test_score(i,1), cov_test_score(i,2),cov_test_score(i,3), '.', 'color', c(timepoint_indices(i - section_length),:))
...
color_bar = colorbar;
color_bar.Ticks = [0 1];
color_bar.TickLabels = {'1', num2str(nframe)};
colormap turbo;
